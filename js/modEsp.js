...
function diagnostico() {
	...
	let indexOfMaxValue = indexOfMax(part1[0]);
	let maxValue = values[indexOfMaxValue];
	let maxValueDisease = names[indexOfMaxValue];

	console.log(
		`Valor maximo: ${maxValue} | Enfermedad con valor maximo: ${maxValueDisease} `
	);

	names.splice(indexOfMaxValue, 1);

	if (maxValue > 1) {
		resCanv.innerHTML = `Entre las enfermedades seleccionadas, la más probable es: ${maxValueDisease}`;
		resCanv2.innerHTML = `Las enfermedades que tienen menos probabilidad de ser son: ${names}`;
	} else {
		resCanv.innerHTML = `Ninguna enfermedad fue detectada con los sintomas ingresados`;
	}
